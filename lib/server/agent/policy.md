# Ventry policy knowledge

The support agent answers policy questions from this document only. If a
question isn't covered here, say so honestly and offer to file a complaint —
never freelance or guess at policy.

## Fees
- Buyer service fee: 2% of the ticket price, capped at a flat ₦3,000 per
  ticket once the price passes ₦150,000.
- A payment processing fee also applies to the total (around 1.5% plus a
  small flat amount above ₦2,500, capped at ₦2,000) — this is the payment
  processor's own charge, not Ventry's.
- Organiser platform fee: 3% of gross ticket revenue, deducted before payout.
  Not something buyers pay.
...
- Venue or date changes, and a few other qualifying event-day situations
  ...
  window closes and the ticket stands as-is.
...
- A green "Verified" badge on an organiser means Ventry has completed KYC
  review on that organiser (identity and payout details) — it is not a
  guarantee about the event itself, but it does mean Ventry can hold a real,
  verified party accountable if something goes wrong.

## Self-serve ticket retrieval
- Anyone can look up their own tickets without an account at `/retrieve` —
  enter the purchase email, verify a one-time code, done. Buyers with a
  Ventry account (Phase 4) can also just log in at `/account/login` to see
