...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: ticket } = await db
      .from('tickets')
      .select('id, buyer_email, refund_code, status, event_id, event:events!tickets_event_id_fkey(event_name)')
    ...
    const eventName = (eventRaw as { event_name: string } | null)?.event_name || 'Unknown Event';

    const { error: insertErr } = await db.from('complaints').insert({
      id: complaintId,
      ticket_id: ticketId,
    ...
    if (insertErr) {
      console.error('POST /api/help/refund: complaint insert error', insertErr);
